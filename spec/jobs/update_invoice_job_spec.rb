...
RSpec.describe UpdateInvoiceJob, type: :job do
  ...
  describe "perform" do
    ...
    it "schedules all the workers" do
      ...
      organization1.save
      organization2.save
      expect(organization3.has_invoice?).to be_falsey

      expect(invoice_active.updated_at).to be_within(1.second).of invoice_active_updated_at
      expect(organization1.has_invoice?).to be_truthy
      ...
      expect(organization2.has_invoice?).to be_truthy
      ...
      expect(invoice_expired.expired?).to be_truthy
      expect(organization2.has_invoice).to be_falsey
      ...
      expect(invoice_to_activate.future?).to be_falsey
      expect(organization3.has_invoice).to be_truthy
    end
  end
end
