...
RSpec.describe FindOrCreateModelAuditJob, type: :job do
  ...
  describe "perform" do
    ...
    it "creates a model_audit" do
      ...
      expect(new_model_audit).to have_attributes basic_target_attributes
    ...
    context "unknown frame_model" do
      ...
      context "bike2 is motorized" do
        ...
        it "matches" do
          ...
          expect(new_model_audit).to have_attributes basic_target_attributes.merge(frame_model: nil, propulsion_type: "pedal-assist")
          expect(UpdateModelAuditJob.jobs.map { |j| j["args"] }.flatten).to eq([new_model_audit.id])
        end
    ...
    context "matching model_audit exists" do
      ...
      it "assigns bike to model_audit" do
        ...
        expect(UpdateModelAuditJob.enqueue_for?(model_audit)).to be_truthy
        expect(UpdateModelAuditJob.jobs.map { |j| j["args"] }.flatten).to eq([model_audit.id])
      end
      context "UpdateModelAuditJob.enqueue_for? false" do
        before { allow(UpdateModelAuditJob).to receive(:enqueue_for?) { false } }
        it "assigns bike, doesn't enqueue" do
          expect_assigned_to_model_audit

          expect(UpdateModelAuditJob.jobs.map { |j| j["args"] }.flatten).to eq([])
        end
      end
    ...
    context "not matching model_audit" do
      ...
      it "enqueues update for model, creates new_model_audit" do
        ...
        expect(new_model_audit).to have_attributes basic_target_attributes.merge(frame_model: "Party MODEL")
        expect(new_model_audit.organization_model_audits.count).to eq 0
        expect(UpdateModelAuditJob.jobs.map { |j| j["args"] }.flatten).to match_array([model_audit.id, new_model_audit.id])
      end
    end
    context "deleted model_audit" do
      it "updates" do
        ...
        new_model_audit = bike1.reload.model_audit
        expect(new_model_audit).to have_attributes basic_target_attributes.merge(cycle_type: "bike")
        expect(UpdateModelAuditJob.jobs.map { |j| j["args"] }.flatten).to match_array([12, new_model_audit.id])
      end
    end
    context "likely_spam and example" do
      it "does not create a model_audit" do
        ...
        expect(UpdateModelAuditJob.jobs.count).to eq 0
      end
    end
