  ...
  context "extra user attributes" do
    ...
    it "assigns the extra user attributes" do
      VCR.use_cassette("after_user_create_worker-import_user_attributes") do
        ...
        expect(Bikes::CalculateStoredLocation.location_attrs(bike)).to match_hash_indifferently target_address_hash.except(:latitude, :longitude).merge(skip_geocoding: false)
