...
RSpec.describe EmailJobs::OwnershipInvitationJob, type: :job do
  ...
  context "ownership does not exist" do
    it "does not send an email" do
      ActionMailer::Base.deliveries = []
      expect {
        EmailJobs::OwnershipInvitationJob.new.perform(129291912)
      }.to change(Notification, :count).by(0)
  ...
  context "creation organization has skip_email" do
    ...
    it "doesn't send email, updates to be send_email false, sends email to the second ownership" do
      ...
      ActionMailer::Base.deliveries = []
      EmailJobs::OwnershipInvitationJob.new.perform(ownership2.id)
      expect(ActionMailer::Base.deliveries.count).to eq 1
    end
  end
end
