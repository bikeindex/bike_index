...
RSpec.describe EmailJobs::PartialRegistrationJob, type: :job do
  ...
  context "a registration the honeypot flagged" do
    ...
    it "sends nothing" do
      ActionMailer::Base.deliveries = []
      expect { EmailJobs::PartialRegistrationJob.new.perform(b_param.id) }.to_not change(Notification, :count)
  ...
  context "partial_register_confirmation" do
    ...
    it "sends the confirmation email, with the token the flow minted" do
      ActionMailer::Base.deliveries = []
      EmailJobs::PartialRegistrationJob.new.perform(b_param.id, "partial_register_confirmation")
      ...
      expect { EmailJobs::PartialRegistrationJob.new.perform(b_param.id, "partial_register_confirmation") }
  ...
  context "a kind no b_param sends" do
    it "raises" do
      ActionMailer::Base.deliveries = []
      expect { EmailJobs::PartialRegistrationJob.new.perform(b_param.id, "finished_registration") }
