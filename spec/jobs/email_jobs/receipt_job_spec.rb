...
RSpec.describe EmailJobs::ReceiptJob, type: :job do
  ...
  it "sends an email" do
    ...
    expect {
      EmailJobs::ReceiptJob.new.perform(payment.id)
    }.to change(EmailJobs::DonationJob.jobs, :count).by 0
  ...
  context "donation, existing notification" do
    ...
    it "enqueues EmailJobs::DonationJob" do
      expect(payment.notifications.count).to eq 1
      ...
      expect {
        EmailJobs::ReceiptJob.new.perform(payment.id)
      }.to change(EmailJobs::DonationJob.jobs, :count).by 1
    ...
    context "notification delivered" do
      ...
      it "does not email" do
        EmailJobs::DonationJob.new.perform(payment.id)
        expect(ActionMailer::Base.deliveries.count).to eq 1
