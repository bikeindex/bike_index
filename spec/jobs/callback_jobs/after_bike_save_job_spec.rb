...
RSpec.describe CallbackJobs::AfterBikeSaveJob, type: :job do
  ...
  describe "enqueuing jobs" do
    ...
    it "enqueues the duplicate_bike_finder_worker" do
      ...
      }.to change(BikeJobs::DuplicateBikeFinderJob.jobs, :size).by 1
    end
  end
  ...
  describe "stolen bike matching a listed serial" do
    ...
    let(:draft_bike) { FactoryBot.create(:bike, :with_ownership_claimed, :with_primary_activity, serial_number: "ABC123XYZ") }
    let!(:draft_listing) { FactoryBot.create(:marketplace_listing, item: draft_bike, updated_at: Time.current - 1.hour) }
    let!(:unmatched_listing) { FactoryBot.create(:marketplace_listing, :for_sale) }

    it "returns the matching for sale listing to draft, leaving the others alone" do
      ...
      expect(draft_listing.reload).to have_attributes(status: "draft", updated_at: be < Time.current - 50.minutes)
    end
  end
  ...
  describe "create_user_registration_organizations" do
    ...
    it "doesn't create" do
      ...
      expect(Sidekiq::Job.jobs.count { |j| j["class"] != "BikeJobs::DuplicateBikeFinderJob" }).to eq 0
      expect(UserRegistrationOrganization.unscoped.count).to eq 0
    ...
    context "with ownership claimed" do
      ...
      it "creates" do
        ...
        Sidekiq::Job.clear_all
        instance.perform(bike.id)
        expect(Sidekiq::Job.jobs.map { |j| j["class"] }.sort).to eq(%w[BikeJobs::DuplicateBikeFinderJob])
  ...
  describe "serialized" do
    ...
    context "with webhook url set" do
      it "calls the things we expect it to call" do
        stub_const("CallbackJobs::AfterBikeSaveJob::POST_URL", "https://example.com")
  ...
  describe "motorized" do
    ...
    it "enqueues FindOrCreateModelAuditJob" do
      ...
      Sidekiq::Job.clear_all
      instance.perform(bike.id)
      expect(Sidekiq::Job.jobs.map { |j| j["class"] }.sort).to eq(%w[BikeJobs::DuplicateBikeFinderJob FindOrCreateModelAuditJob])
