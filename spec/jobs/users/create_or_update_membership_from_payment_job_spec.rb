...
RSpec.describe Users::CreateOrUpdateMembershipFromPaymentJob, type: :job do
  ...
  it "creates a membership" do
    ...
    expect(payment.membership).to have_attributes new_attrs
  end
  ...
  context "when amount is for $99" do
    ...
    it "creates a membership" do
      ...
      expect(payment.membership).to have_attributes new_attrs.merge(level: "patron", end_at: Time.current + 1.year)
    end
  end
  ...
  context "when user already has a membership" do
    ...
    it "extends the membership" do
      ...
      expect(membership.reload).to have_attributes updated_attrs
    end
