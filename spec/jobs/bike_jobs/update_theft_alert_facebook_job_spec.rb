...
RSpec.describe BikeJobs::UpdateTheftAlertFacebookJob, type: :job do
  ...
  describe "perform" do
    ...
    it "updates the promoted_alert and sends a message" do
      ...
      expect {
        instance.perform
      }.to change(BikeJobs::UpdateTheftAlertFacebookJob.jobs, :count).by 1
    ...
    context "pending" do
      ...
      it "enqueues activate theft worker and exits" do
        ...
        expect {
          instance.perform
        }.to change(BikeJobs::UpdateTheftAlertFacebookJob.jobs, :count).by 0
        # But if it's processed, it runs activate
        expect {
          instance.perform(promoted_alert.id)
        }.to change(BikeJobs::ActivateTheftAlertJob.jobs, :count).by 1
      end
      context "failed_to_activate" do
        ...
        it "enqueues activate theft worker and exits" do
          ...
          expect(promoted_alert.manual_override_inactive?).to be_falsey
          expect(promoted_alert.should_update_facebook?).to be_falsey
        ...
        context "manual_override_inactive?" do
          ...
          it "does not enqueue" do
            ...
            expect(promoted_alert.send(:failed_to_activate_data?)).to be_truthy
            ...
            expect {
              instance.perform(promoted_alert.id)
            }.to change(BikeJobs::ActivateTheftAlertJob.jobs, :count).by 0
          end
        end
      end
    end
