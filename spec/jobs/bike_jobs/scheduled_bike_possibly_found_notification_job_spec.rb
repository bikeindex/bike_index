...
RSpec.describe BikeJobs::ScheduledBikePossiblyFoundNotificationJob, type: :job do
  ...
  context "given no bikes with matches" do
    it "does not enqueue any notification jobs" do
      ...
      expect(EmailJobs::BikePossiblyFoundNotificationJob.jobs.length).to eq(0)
  ...
  context "given bikes with matches" do
    it "enqueues a notification job for each match" do
      ...
      expect(EmailJobs::BikePossiblyFoundNotificationJob.jobs.length).to eq(2)
    end
  end
end
