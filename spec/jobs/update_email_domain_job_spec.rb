  ...
  describe "perform" do
    ...
    context "bikeindex" do
      ...
      it "updates counts in the cache" do
        VCR.use_cassette("Update-Email-Domain-Job_bikeindex") do
          ...
          expect(email_domain.data.except("spam_score")).to have_attributes valid_data.merge(broader_domain_exists: true, subdomain_count: 1)
          ...
          expect(email_domain_bare.data.except("spam_score")).to have_attributes(valid_data.merge(subdomain_count: 1, no_auto_assign_status: true))
        end
      end
    end
    ...
    context "invalid_domain" do
      ...
      it "updates counts in the cache" do
        ...
        expect(email_domain.data.except("spam_score")).to have_attributes invalid_data
      end
    end
    ...
    context "@gmail.com" do
      ...
      it "resolves" do
        VCR.use_cassette("Update-Email-Domain-Job_gmail") do
          ...
          expect(email_domain.data.except("spam_score")).to have_attributes target_data
          expect(email_domain.status).to eq "permitted"
      ...
      it "doesn't resolve, but bike makes it permitted" do
        VCR.use_cassette("Update-Email-Domain-Job_co.za") do
          ...
          expect(email_domain.data.except("sendgrid_validations", "spam_score"))
            .to have_attributes valid_data.merge(domain_resolves: false, tld_resolves: false)
    ...
    context "@msn.com" do
      ...
      it "doesn't resolve, but bike makes it permitted" do
        ...
        VCR.use_cassette("Update-Email-Domain-Job_msn") do
          ...
          expect(email_domain.data.except("spam_score")).to have_attributes msn_data
          expect(email_domain.status).to eq "permitted"
          expect(email_domain.ban_blockers.any?).to be_truthy
    ...
    context "domain that doesn't resolve" do
      ...
      before do
        expect(user2).to be_present # So EMAIL_MIN_COUNT passes
      end
      ...
      it "makes provisional_ban" do
        ...
        VCR.use_cassette("Update-Email-Domain-Job_unresolved") do
          ...
          expect(email_domain.ban_blockers.any?).to be_falsey
          expect(email_domain.data.except("spam_score")).to have_attributes target_data
          expect(email_domain.status).to eq "provisional_ban"
      ...
      context "create_tld_subdomain_count" do
        ...
        it "makes tld domain" do
          VCR.use_cassette("Update-Email-Domain-Job_unresolved") do
            ...
            instance.perform(email_domain.id)
            expect(email_domain.reload.ban_blockers.any?).to be_falsey
            ...
            expect(email_domain_tld.reload.ban_blockers).to eq(["permitted_subdomains"])
            expect(email_domain_tld.reload.status).to eq "permitted"
            expect(described_class.jobs.map { |j| j["args"] }.flatten).to eq([])
            expect(email_domain_tld.calculated_users.count).to eq 2

            email_domain_tld.update(status: "provisional_ban", data: email_domain.data.merge(no_auto_assign_status: true))
            instance.perform(email_domain_tld.id)
