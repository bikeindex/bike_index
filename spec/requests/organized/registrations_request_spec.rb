...
RSpec.describe Organized::RegistrationsController, type: :request do
  ...
  describe "index" do
    ...
    it "leaves impounded bikes out by default, and takes a status it permits" do
      ...
      expect(assigns(:search_status)).to eq "stolen"
    end

    context "without impound_bikes" do
      let(:enabled_feature_slugs) { %w[bike_search] }

      it "defaults to every status, and refuses the impound ones" do
        get base_url, params: {search_no_js: true}
