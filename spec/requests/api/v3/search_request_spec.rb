...
RSpec.describe "Search API V3", type: :request do
  ...
  describe "/" do
    ...
    context "with per_page" do
      it "returns matching bikes, defaults to stolen" do
        ...
        get "/api/v3/search", params: query_params.merge(per_page: 1), headers: { format: :json }
  ...
  describe "/close_serials" do
    ...
    context "with per_page" do
      it "returns matching bikes, defaults to stolen" do
        get "/api/v3/search/close_serials", params: query_params, headers: { format: :json }
        result = JSON.parse(response.body)
  ...
  describe "/count" do
    ...
    context "correct params" do
      ...
      it "calls Bike Search with the expected interpreted_params" do
        VCR.use_cassette("v3_bike_search-count") do
          get "/api/v3/search/count", params: request_query_params, headers: { format: :json }
          result = JSON.parse(response.body)
    ...
    context "nil params" do
      it "succeeds" do
        get "/api/v3/search/count", params: { stolenness: "", query_items: [], serial: "" }, headers: { format: :json }
    ...
    context "with query items" do
      ...
      let(:query_params) { { query_items: [manufacturer.search_id] } }
      it "succeeds" do
        get "/api/v3/search/count", params: query_params, headers: { format: :json }
        result = JSON.parse(response.body)
