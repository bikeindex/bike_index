  ...
  describe "check_if_registered" do
    ...
    context "user is organization member" do
      ...
      it "returns true" do
        ...
        post check_if_registered_url, params: required_params.merge(manufacturer: "Unknown manufacturer party").to_json, headers: json_headers
        expect(response.code).to eq("400")
        expect(json_result["error"]).to match(/manufacturer/i)

        # It doesn't match when passed optional attribute that doesn't match
        post check_if_registered_url, params: search_params.merge(cycle_type: "tandem").to_json, headers: json_headers
        expect(response.code).to eq("201")
        expect_hashes_to_match(json_result, target_result.merge(registered: false))
        post check_if_registered_url, params: search_params.merge(color: FactoryBot.create(:color).name).to_json, headers: json_headers
        expect(response.code).to eq("201")
        expect_hashes_to_match(json_result, target_result.merge(registered: false))
