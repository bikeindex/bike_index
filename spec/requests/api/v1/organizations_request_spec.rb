...
RSpec.describe API::V1::OrganizationsController, type: :request do
  ...
  describe "show" do
    ...
    context "numeric id, has_bike_stickers" do
      let(:organization) { FactoryBot.create(:organization_with_auto_user, :in_chicago_legacy, kind: "bike_shop") }
      let!(:organization_regional_parent) { FactoryBot.create(:organization_with_organization_features, :in_chicago_legacy, regional_ids: [organization.id], enabled_feature_slugs: %w[regional_bike_counts bike_stickers]) }
      let(:target) do
        {
          id: organization.id,
