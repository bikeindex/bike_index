  ...
  context "no existing b_param, reg_address, top_level_propulsion_type" do
    ...
    it "creates with address", :flaky do
      ...
      VCR.use_cassette("bikes_controller-create-reg_address", match_requests_on: [:method]) do
        expect(BikeService::Displayer.display_edit_address_fields?(Bike.new, current_user)).to be_truthy
        organization.reload
        ...
        expect(BikeService::Displayer.display_edit_address_fields?(new_bike, current_user)).to be_falsey
