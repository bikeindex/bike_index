...
RSpec.describe "BikesController#create", type: :request do
  ...
  context "blank serials" do
    ...
    context "scanned_sticker" do
      ...
      it "assigns scanned_sticker", :flaky do
        ...
        expect(new_bike.address_record).to have_attributes default_location_address_record_attrs.merge(kind: "ownership")
  ...
  context "no existing b_param, reg_address, top_level_propulsion_type" do
    ...
    it "does not have address, has association", :flaky do
      ...
      expect(flash[:success]).to be_present
      new_bike = Bike.last
      expect(new_bike).to have_attributes testable_bike_params.merge(cycle_type: "non-e-scooter")
      ...
      expect(new_bike.address_record).to have_attributes(address_attrs)
    ...
    context "with address_record_attributes" do
      ...
      it "creates with address", :flaky do
        ...
        VCR.use_cassette("bikes_controller-create-reg_address", match_requests_on: [:method]) do
          ...
          expect(new_bike.address_record).to have_attributes target_address_record_attributes
          ...
          expect(ownership.registration_info).to have_attributes reg_hash
  ...
  context "existing b_param, no bike" do
    ...
    it "creates the bike and does the updated address thing" do
      ...
      expect(new_bike.address_record).to have_attributes target_address
      ...
      expect(new_bike.registration_info).to have_attributes(target_registration_info)
    ...
    context "legacy address attributes" do
      ...
      it "creates the bike and does the updated address thing" do
        ...
        expect(new_bike.phone).to eq "18887776666"
        expect(new_bike.student_id).to eq "999888"
        expect(new_bike.registration_info).to have_attributes(target_registration_info.except(:region_string, :postal_code))
