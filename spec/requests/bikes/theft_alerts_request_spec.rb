...
RSpec.describe Bikes::TheftAlertsController, type: :request, vcr: true, match_requests_on: [:method], re_record_interval: re_record_interval do
  ...
  describe "create" do
    ...
    def expect_promoted_alert_to_be_created
      ...
      expect(promoted_alert.theft_alert_plan_id).to eq theft_alert_plan.id
  ...
  describe "show" do
    ...
    let(:promoted_alert) { FactoryBot.create(:promoted_alert, theft_alert_plan: theft_alert_plan, stolen_record: bike.current_stolen_record) }
    let(:payment) { Payment.create(stripe_id: stripe_id, user: current_user, payment_method: "stripe", amount: nil, kind: "theft_alert", promoted_alert:) }
    it "marks as paid" do
    ...
    context "with an activateable promoted_alert" do
      let(:bike) { FactoryBot.create(:bike, :with_ownership_claimed, user: current_user) }
      ...
      it "marks as paid and enqueues activation" do
        ...
        expect(payment.reload.paid?).to be_falsey
        expect(payment.amount_cents).to eq 0
        expect(promoted_alert.reload.activateable?).to be_falsey
        expect(promoted_alert.missing_photo?).to be_falsey
        expect(promoted_alert.bike_not_current?).to be_falsey
