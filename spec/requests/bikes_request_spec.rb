...
RSpec.describe BikesController, type: :request do
  ...
  describe "create" do
    ...
    context "no existing b_param, impounded" do
      ...
      context "valid" do
        let(:bike_params) { {manufacturer_id: manufacturer.name, serial_number: "found bike serial", year: "2022", frame_model: "Cool frame model", primary_frame_color_id: color.id.to_s} }
        ...
        it "creates a new ownership and impound_record" do
          VCR.use_cassette("bikes_controller-create-impound-chicago", match_requests_on: [:path]) do
            ...
            testable_bike_params.each { |k, v| expect(new_bike.send(k).to_s).to eq v.to_s }
            impound_record = new_bike.current_impound_record
            expect(impound_record).to be_present
            stolen_params.except(:date_stolen, :timezone).each { |k, v| expect(impound_record.send(k).to_s).to eq v.to_s }
            expect(impound_record.date_stolen.to_i).to be_within(1).of(Time.current.yesterday.to_i)
            expect(impound_record.show_address).to be_falsey
          end
        end
      end
    end
    context "existing b_param, no bike" do
      ...
      it "creates a bike" do
        ...
        bike_params.each { |k, v| expect(new_bike.send(k).to_s).to eq v }
