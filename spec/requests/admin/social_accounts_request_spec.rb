...
RSpec.describe Admin::SocialAccountsController, type: :request, vcr: true do
  ...
  describe "update" do
    ...
    it "updates without check_credentials" do
      ...
      expect_any_instance_of(SocialAccount).to_not receive(:platform_client)
    ...
    context "switching to active" do
      let(:platform_client) { OpenStruct.new(verify_credentials: false) }
      it "updates and checks_credentials" do
        expect(social_account.errored?).to be_truthy
        allow_any_instance_of(SocialAccount).to receive(:platform_client) { platform_client }
        expect(platform_client).to receive(:verify_credentials) { true }
        patch "#{base_url}/#{social_account.id}",
          params: {
  ...
  describe "#destroy" do
    context "given a successful deletion" do
      it "deletes the account, redirects to account index url with an appropriate flash" do
        social_account = FactoryBot.create(:social_account_1)
    ...
    context "given a failed deletion" do
      it "redirects to account edit url with an appropriate flash" do
        social_account = FactoryBot.create(:social_account_1)
