...
RSpec.describe Admin::BikesController, type: :request do
  ...
  describe "index" do
    ...
    context "with search_description" do
      let!(:match_bike) { FactoryBot.create(:bike, :with_ownership, description: "unique-desc-xyz") }
      let!(:other_bike) { FactoryBot.create(:bike, :with_ownership, description: "something else") }
      it "filters by description" do
        get base_url, params: {search_description: "unique-desc-xyz"}
        expect(response.code).to eq("200")
        expect(assigns(:bikes).pluck(:id)).to include(match_bike.id)
        expect(assigns(:bikes).pluck(:id)).not_to include(other_bike.id)
      end
    end

    context "with search_person" do
      let(:ownership) { match_bike.current_ownership }
      let!(:match_bike) { FactoryBot.create(:bike, :with_ownership) }
      let!(:other_bike) { FactoryBot.create(:bike, :with_ownership) }

      it "matches on owner_email" do
        email = match_bike.owner_email
        get base_url, params: {search_person: email}
        expect(response.code).to eq("200")
        expect(assigns(:bikes).pluck(:id)).to include(match_bike.id)
        expect(assigns(:bikes).pluck(:id)).not_to include(other_bike.id)
      end

      it "matches on owner_name" do
        match_bike.current_ownership.update!(owner_name: "Alejandra Unique-Name")
        get base_url, params: {search_person: "Alejandra Unique-Name"}
        expect(response.code).to eq("200")
        expect(assigns(:bikes).pluck(:id)).to include(match_bike.id)
        expect(assigns(:bikes).pluck(:id)).not_to include(other_bike.id)
      end

      it "matches on student_id in registration_info" do
        match_bike.current_ownership.update!(registration_info: {"student_id" => "STUD-99887"})
        get base_url, params: {search_person: "STUD-99887"}
        expect(response.code).to eq("200")
        expect(assigns(:bikes).pluck(:id)).to include(match_bike.id)
        expect(assigns(:bikes).pluck(:id)).not_to include(other_bike.id)
      end
    ...
    context "with search_notes" do
      ...
      it "filters by note body" do
        FactoryBot.create(:bike_organization_note, bike: match_bike, organization:, body: "notable-search-term-abc")
        get base_url, params: {search_notes: "notable-search-term-abc"}
        expect(response.code).to eq("200")
        expect(assigns(:bikes).pluck(:id)).to include(match_bike.id)
        expect(assigns(:bikes).pluck(:id)).not_to include(other_bike.id)
      end

      it "returns each matching bike once even when multiple notes match" do
        org2 = FactoryBot.create(:organization)
        FactoryBot.create(:bike_organization_note, bike: match_bike, organization:, body: "dup-search-term")
        FactoryBot.create(:bike_organization_note, bike: match_bike, organization: org2, body: "dup-search-term")
        get base_url, params: {search_notes: "dup-search-term"}
        expect(response.code).to eq("200")
        expect(assigns(:bikes).pluck(:id).count(match_bike.id)).to eq(1)
      end
    end
  end
