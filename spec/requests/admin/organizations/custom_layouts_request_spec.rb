...
RSpec.describe Admin::Organizations::CustomLayoutsController, type: :request do
  ...
  context "super admin and developer" do
    ...
    describe "edit" do
      # The tabs lead to admin pages the legacy bundle sets up on DOM ready, which a turbo
      # restoration visit doesn't re-run - it restores a clone of its snapshot, so coming
      # back would find live-looking markup nothing is bound to
      it "doesn't send the tabs through turbo" do
        get "#{base_url}/landing_page/edit"
        expect(response.status).to eq(200)
        expect(response.body).to include("data-ui--forms--file-upload-target=\"list\"")
        expect(response.body).to include("data-ui--forms--file-upload-params-value=\"{&quot;organization_id&quot;:&quot;#{organization.to_param}&quot;}\"")
        ...
        context "with a snippet" do
          ...
          it "links to the version history" do
            ...
            expect(response.body).to include CGI.escapeHTML(
              admin_paper_trail_versions_path(search_item_type: "MailSnippet",
