...
RSpec.describe PageBlock::MyAccount::OrganizationRoles::Component, type: :component do
  ...
  context "with a second organization" do
    ...
    it "renders a row for each role" do
      rows = component.css("[data-page-block--my-account--organization-roles-target='item']")
  ...
  context "with only one role" do
    it "drops the ordering copy and the drag handle" do
      expect(component).to have_css("[data-page-block--my-account--organization-roles-target='item']", count: 1)
      expect(component).to_not have_css("[data-page-block--my-account--organization-roles-target='handle']")
