...
RSpec.describe Pages::Org::Search::ChartCard::Component, type: :component do
  ...
  it "defaults to the year scope, captioning it and rendering the stats" do
    ...
    expect(component).to have_css("button[aria-describedby]", exact_text: "+100%")
    expect(component).to have_css("[role=tooltip]", exact_text: "6 last year", visible: :all)
  ...
  context "with the search scope" do
    ...
    it "captions the search" do
      ...
      expect(component).to have_css("[role=tooltip]", exact_text: "6 the period before", visible: :all)
    end
  end
