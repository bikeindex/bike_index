...
RSpec.describe Pages::MyAccount::OrganizationRoles::Component, type: :component do
  ...
  context "with a second organization" do
    ...
    it "renders a row for each role" do
      rows = component.css("[data-my-account--organization-roles-target='item']")
  ...
  context "with only one role" do
    it "drops the ordering copy and the drag handle" do
      expect(component).to have_css("[data-my-account--organization-roles-target='item']", count: 1)
      expect(component).to_not have_css("[data-my-account--organization-roles-target='handle']")
  ...
  describe "leave organization" do
    ...
    it "renders the button" do
      ...
      # data-turbo too: turbo_method alone is silently inert
