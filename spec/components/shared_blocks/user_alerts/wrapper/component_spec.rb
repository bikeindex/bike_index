...
RSpec.describe SharedBlocks::UserAlerts::Wrapper::Component, type: :component do
  ...
  it "has a component, a preview scenario and a spec for every kind of alert" do
    ...
    alert_names = Rails.root.glob("app/components/shared_blocks/user_alerts/*").select(&:directory?)
      .map { |dir| dir.basename.to_s }.sort - %w[bike_list_modal wrapper]
    scenarios = SharedBlocks::UserAlerts::Wrapper::ComponentPreview.public_instance_methods(false).map(&:to_s)
    ...
    alert_names.each do |alert|
      ...
      spec = Rails.root.join("spec/components/shared_blocks/user_alerts/#{alert}/component_spec.rb")
