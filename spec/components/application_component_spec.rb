...
RSpec.describe ApplicationComponent, type: :component do
  describe "sidecar translations" do
    # calculated_markup_digest globs a component's own directory, so a sidecar one level
    # up contributes to no digest and its copy can go stale inside a fragment cache —
    # silently, since the digest spec recomputes through that same glob
