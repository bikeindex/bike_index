...
RSpec.describe UI::Table::Component, type: :component do
  ...
  context "with sortable columns" do
    ...
    it "renders sortable headers with link class and active state" do
      result = render_inline(described_class.new(records:, render_sortable: true, sort_state: SortState.new(sort: "name", direction: "desc"))) do |table|
    ...
    context "with custom label" do
      it "uses label instead of derived title" do
        result = render_inline(described_class.new(records:, render_sortable: true, sort_state: SortState.new(sort: "bike_sticker_batch_id"))) do |table|
    ...
    context "with sort_indicator on a non-sortable column" do
      it "renders the sort arrow without a link when matching current sort" do
        result = render_inline(described_class.new(records:, render_sortable: true, sort_state: SortState.new(sort: "created_at", direction: "desc"))) do |table|
      ...
      it "does not render an arrow when sort_indicator does not match current sort" do
        result = render_inline(described_class.new(records:, render_sortable: true, sort_state: SortState.new(sort: "email"))) do |table|
          table.column(sortable: "email") { |r| r.email }
