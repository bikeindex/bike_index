...
RSpec.describe UI::PhoneDisplay::Component, type: :component do
  ...
  it "renders a tel link" do
    expect(instance.render?).to be_truthy
    expect(component).to eq '<a href="tel:999-999-9999">999-999-9999</a>'
  end

  context "with a country code" do
    let(:phone) { "+91 8041505583" }
    it "renders the country code" do
      expect(component).to eq '<a href="tel:+91-804-150-5583">+91-804-150-5583</a>'
    end
  end
  ...
  context "with an extension" do
    ...
    it "pauses before the extension in the link" do
      expect(component).to eq '<a href="tel:+11-121-1111 ; 2929222">+11-121-1111 x 2929222</a>'
    end
  end

  context "with html_options" do
    let(:options) { {phone: "777 777 7777 ext. 2929222", class: "phone-number-link"} }
    it "passes them to the link" do
      expect(component).to eq '<a class="phone-number-link" href="tel:777-777-7777 ; 2929222">777-777-7777 x 2929222</a>'
    end
  end

  context "skip_link" do
    let(:options) { {phone:, skip_link: true} }
    it "renders the number without a link" do
      expect(component).to eq "<span>999-999-9999</span>"
    end

    context "with html_options" do
      let(:options) { {phone:, skip_link: true, class: "tw:font-mono"} }
      it "passes them to the span" do
    ...
    context "with html in the extension" do
      ...
      it "escapes it" do
        expect(component).to eq "<span>777-777-7777 x &lt;script&gt;alert(1)&lt;/script&gt;</span>"
      end
    end
  end
