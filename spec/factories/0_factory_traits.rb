...
FactoryBot.define do
  ...
  trait :in_nyc do
    ...
    city { "New York" }
    region_record_id { FactoryBot.create(:state_new_york).id }
    country_id { Country.united_states.id }
    street { "278 Broadway" }
    postal_code { "10007" }
  end
  ...
  # Legacy location traits using Geocodeable-style columns (state_id/zipcode)
  # for models that still use the Geocodeable concern
  trait :in_amsterdam_legacy do
    latitude { 52.37403 }
    longitude { 4.88969 }
    city { "Amsterdam" }
    state_id { nil }
