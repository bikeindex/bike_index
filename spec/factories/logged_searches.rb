FactoryBot.define do
  factory :logged_search do
    request_at { Time.current - 5.minutes }
    request_id { "6473c6f5-51f6-422b-bb3c-7e94b670f520" }
    log_line do
      "I, [#{request_at.utc.strftime("%FT%T%:z")} #6669]  INFO -- : [#{request_id}] " \
      "{\"method\":\"GET\",\"path\":\"/bikes\",\"format\":\"html\"," \
      "\"controller\":\"BikesController\",\"action\":\"index\",\"status\":200," \
      "\"duration\":1001.79,\"view\":52.68,\"db\":946.26,\"remote_ip\":\"11.222.33.4\"," \
      "\"u_id\":null,\"params\":{\"non_stolen\":\"\",\"page\":\"2\"," \
      "\"proximity\":\"New Orleans Louisiana\",\"proximity_radius\":\"100\","
      "\"query\":\"\",\"stolen\":\"true\"},\"@timestamp\":\"2023-10-23T00:20:01.681Z\"," \
      "\"@version\":\"1\",\"message\":\"[200] GET /bikes (BikesController#index)\"}}"
    end
  end
end
