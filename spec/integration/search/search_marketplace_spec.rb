...
RSpec.describe "Marketplace infinite scroll", :js, type: :system do
  ...
  it "automatically loads the next page when scrolling to bottom" do
    ...
    # Pagination links are the fallback for users without JS - here they're gone,
    # and the spinner they ship hidden alongside is showing instead
    expect(page).to have_no_css("[data-search--pagination-fallback-target='links']", visible: :all)
    expect(page).to have_css("[data-search--pagination-fallback-target='spinner']")
    scroll_to_lazy_load
