...
# can be rendered (e.g. `?bike_id=1234`). Without an arg, the lookup uses
# the same heuristics as before — usually `Model.last`.
class OrganizedMailerPreview < ActionMailer::Preview
  def graduated_notification(graduated_notification_id: nil)
  ...
  def parking_notification(parking_notification_id: nil)
  ...
  def impound_claim_approved_or_denied(impound_claim_id: nil)
