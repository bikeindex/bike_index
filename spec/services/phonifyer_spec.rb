...
RSpec.describe Phonifyer do
  describe "phonify" do
    ...
    context "no country code" do
      it "strips and remove non digits" do
        expect(Phonifyer.send(:strip_ignored_parts, "(999) 899 - 999")).to eq "999899999"
        expect(Phonifyer.send(:split_with_country_code, "(999) 899 - 999")).to eq(["(999) 899 - 999"])
        expect(Phonifyer.send(:split_with_extension, "(999) 899 - 999")).to eq(["(999) 899 - 999"])
    ...
    describe "country_code parsing" do
      it "does not remove the 1" do
        expect(Phonifyer.send(:split_with_country_code, "+1 80 4150 5583")).to eq(["80 4150 5583", "+1"])
        expect(Phonifyer.send(:split_with_country_code, "+18041505583")).to eq(["8041505583", "+1"])
        expect(Phonifyer.send(:split_with_country_code, "+1 8041505583")).to eq(["8041505583", "+1"])
      ...
      it "does not remove the 881" do
        expect(Phonifyer.send(:split_with_country_code, "+881 80 4150 5583")).to eq(["80 4150 5583", "+881"])
        expect(Phonifyer.send(:split_with_country_code, "+8818041505583")).to eq(["8041505583", "+881"])
        # And test the whole phonify works
      ...
      it "does not remove the 91" do
        expect(Phonifyer.send(:split_with_country_code, "+91 80 4150 5583")).to eq(["80 4150 5583", "+91"])
        expect(Phonifyer.send(:split_with_country_code, "+918041505583")).to eq(["8041505583", "+91"])
        # And test the whole phonify works
      ...
      it "does not remove 35" do
        expect(Phonifyer.send(:split_with_country_code, "+35-3871730000")).to eq(["-3871730000", "+35"])
        expect(Phonifyer.send(:split_with_country_code, "+35(387)173-0000")).to eq(["(387)173-0000", "+35"])
        expect(Phonifyer.send(:split_with_country_code, "+35 3871730000")).to eq(["3871730000", "+35"])
        # And test the whole phonify works
      ...
      it "does not remove 44" do
        expect(Phonifyer.send(:split_with_country_code, "+44 780 273 0000")).to eq(["780 273 0000", "+44"])
        expect(Phonifyer.send(:split_with_country_code, "+44 (780) 273-0000")).to eq(["(780) 273-0000", "+44"])
        expect(Phonifyer.send(:split_with_country_code, "+447802730000 ext.121222")).to eq(["7802730000 ext.121222", "+44"])
        # And test the whole phonify works
    ...
    context "with extension" do
      it "includes x" do
        expect(Phonifyer.send(:split_with_extension, "800.478.2111 x1111")).to eq(["800.478.2111", "x1111"])
        expect(Phonifyer.send(:split_with_extension, "800.478.2111x1111")).to eq(["800.478.2111", "x1111"])
        expect(Phonifyer.send(:split_with_extension, "8004782111 ext 1111")).to eq(["8004782111", "x1111"])
        expect(Phonifyer.send(:split_with_extension, "(800)478-2111 ext. 1111")).to eq(["(800)478-2111", "x1111"])
        expect(Phonifyer.send(:split_with_extension, "8004782111 EXT.1111")).to eq(["8004782111", "x1111"])
        expect(Phonifyer.send(:split_with_extension, "800478.2111 extension 1111")).to eq(["800478.2111", "x1111"])
        # And test the whole phonify works
  ...
  describe "components" do
    it "separates into components" do
      ...
      expect(Phonifyer.components("+411111111")).to eq({country_code: "4", number: "11111111"})
    end
