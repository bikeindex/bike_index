...
RSpec.describe CspReport do
  ...
  describe "forwardable" do
    let(:body) { {"csp-report" => report}.to_json }

    it "returns the normalized report" do
      expect(described_class.forwardable(body, user_agent)).to eq report
    end

    context "blocked-uri with a query string" do
      let(:blocked_uri) { "https://evil.example.com/x.js?label=WADeCPKg7gYQtvfc0wM" }

      it "drops the query" do
        expect(described_class.forwardable(body, user_agent)["blocked-uri"]).to eq "https://evil.example.com/x.js"
      end
    end
    ...
    context "noise" do
      ...
      it "returns nil" do
        expect(described_class.forwardable(body, user_agent)).to be_nil
      end
    end

    it "returns nil for a malformed body" do
      expect(described_class.forwardable("not json", user_agent)).to be_nil
    end
  end

  describe "parse" do
    it "pulls the report out of the envelope" do
      expect(described_class.send(:parse, {"csp-report" => report}.to_json)).to eq report
    end

    ["not json", "null", "123", {"csp-report" => 5}.to_json].each do |raw_body|
      it "returns nil for #{raw_body.inspect}" do
        expect(described_class.send(:parse, raw_body)).to be_nil
      end
    end
  end
  ...
  describe "normalize" do
    it "leaves a uri without a query alone" do
      expect(described_class.send(:normalize, report)).to eq report
    end
    ...
    context "blocked-uri with a query string" do
      ...
      it "drops the query, so the fault doesn't fingerprint per-request" do
        expect(described_class.send(:normalize, report)["blocked-uri"]).to eq "https://evil.example.com/x.js"
      end
    end
  ...
  describe "noise?" do
    it "is false for a violation we can act on" do
      expect(described_class.send(:noise?, report, user_agent)).to be_falsey
    end

    context "browser-extension noise" do
      let(:blocked_uri) { "chrome-extension://0dca8e62/fonts/Inter-Variable.ttf" }
      it "is true" do
        expect(described_class.send(:noise?, report, user_agent)).to be_truthy
      end
    end
