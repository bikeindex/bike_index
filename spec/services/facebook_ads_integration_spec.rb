...
if !ENV["CI"] && facebook_imported && Facebook::AdsIntegration::TOKEN.present?
  RSpec.describe Facebook::AdsIntegration do
    ...
    context "with promoted_alert" do
      ...
      let(:promoted_alert) do
        PromotedAlert.new(id: 12, theft_alert_plan: theft_alert_plan,
          stolen_record: stolen_record,
          latitude: 37.8297171, longitude: -122.2803456,
          facebook_data: {campaign_id: campaign_id, adset_id: adset_id})
      end
      before do
        # Required because default scope override in promoted_alert
        allow(promoted_alert).to receive(:stolen_record) { stolen_record }
        allow(promoted_alert).to receive(:facebook_name) { "New Test Theft Alert" }
      end

      describe "create_campaign" do
        let(:promoted_alert) { PromotedAlert.new(id: 12) }
        it "creates a campaign" do
          expect(promoted_alert.address_string).to eq "10000 138 st, Edmonton, T5N 2H7, Canada"
          VCR.use_cassette("facebook/ads_integration-create_campaign", match_requests_on: [:method]) do
            campaign = instance.create_campaign(promoted_alert)
      ...
      describe "create_adset" do
        it "creates an adset" do
          expect(promoted_alert.campaign_id).to eq campaign_id
          VCR.use_cassette("facebook/ads_integration-create_adset", match_requests_on: [:method]) do
            adset = instance.create_adset(promoted_alert)
      ...
      describe "create_ad, create_for" do
        ...
        it "creates an adset" do
          expect(promoted_alert.bike).to eq bike
          expect(promoted_alert.adset_id).to eq adset_id
          expect(promoted_alert.message).to eq message
          VCR.use_cassette("facebook/ads_integration-create_ad", match_requests_on: [:method]) do
            ad = instance.create_ad(promoted_alert)
        ...
        describe "create_for" do
          let(:promoted_alert) { FactoryBot.create(:promoted_alert, theft_alert_plan: theft_alert_plan) }
          it "creates an ad and saves the data" do
            expect(promoted_alert.message).to eq message
            expect(promoted_alert).to be_valid
            ...
            VCR.use_cassette("facebook/ads_integration-create_for", match_requests_on: [:method]) do
              instance.create_for(promoted_alert)
              promoted_alert.reload
              expect(promoted_alert.campaign_id).to be_present
              expect(promoted_alert.adset_id).to be_present
              expect(promoted_alert.ad_id).to be_present
              expect(promoted_alert.facebook_data["objective_campaign"]).to eq "OUTCOME_AWARENESS"
              expect(promoted_alert.objective_campaign).to eq "OUTCOME_AWARENESS"
              expect(promoted_alert.facebook_data["objective_adset"]).to eq "REACH"
              expect(promoted_alert.objective_adset).to eq "REACH"
              # This fails when it's freshly created - so ignoring
              # expect(promoted_alert.facebook_post_url).to be_present
            end
            expect(PromotedAlert.matching_adset_objective("OUTCOME_AWARENESS").pluck(:id)).to eq([promoted_alert.id])
            expect(PromotedAlert.matching_adset_objective("REACH").pluck(:id)).to eq([promoted_alert.id])
          end
        end
        ...
        #         expect(adset).to be_present
        #         pp adset.id
        #         raise "Put in the new adset ID: #{adset.id}, so a promoted_alert can be created from it"
        #       end
        #       ad = instance.create_ad(promoted_alert)
        #       expect(ad).to be_present
        #       expect(ad.id).to be_present
      ...
      describe "update_facebook_data" do
        ...
        let(:promoted_alert) do
          # id: 1633
          PromotedAlert.create(theft_alert_plan: theft_alert_plan,
            stolen_record: stolen_record,
            user: bike.user,
        ...
        it "updates and sets the data" do
          expect(promoted_alert).to be_valid
          expect(promoted_alert.id).to be_present
          expect(promoted_alert.facebook_data).to match_hash_indifferently facebook_data
          expect(promoted_alert.reload.reach).to be_blank
          VCR.use_cassette("facebook/ads_integration-update_facebook_data", match_requests_on: [:method]) do
            instance.update_facebook_data(promoted_alert)
            promoted_alert.reload
            expect(promoted_alert.facebook_updated_at).to be_within(2).of Time.current
            expect(promoted_alert.facebook_data["effective_object_story_id"]).to eq effective_object_story_id
            expect(promoted_alert.facebook_data["amount_cents"]).to eq amount_cents_facebook
            expect(promoted_alert.facebook_data["spend_cents"].to_i).to eq 3778
            expect(promoted_alert.amount_cents_facebook_spent).to eq 3778
            expect(promoted_alert.reach).to eq 20_899
            expect(promoted_alert.engagement).to match_hash_indifferently target_engagement
          end
        end
        context "ad completed running finished ad" do
          ...
          it "updates and sets the data" do
            ...
            VCR.use_cassette("facebook/ads_integration-update_facebook_data-2", match_requests_on: [:method]) do
              ...
              promoted_alert.reload
              expect(promoted_alert.facebook_updated_at).to be_within(2).of Time.current
              expect(promoted_alert.facebook_data["effective_object_story_id"]).to eq facebook_data[:effective_object_story_id]
              expect(promoted_alert.facebook_data["amount_cents"]).to eq amount_cents_facebook
              expect(promoted_alert.facebook_data["spend_cents"]).to eq 3793.0
              expect(promoted_alert.reach).to eq 16_092
              expect(promoted_alert.engagement).to be_present
            end
          end
        end
        context "insights missing unique_clicks" do
          ...
          it "sets engagement with a nil unique_clicks" do
            VCR.use_cassette("facebook/ads_integration-update_facebook_data-no_unique_clicks", match_requests_on: [:method]) do
              expect { instance.update_facebook_data(promoted_alert) }.not_to raise_error
              promoted_alert.reload
              expect(promoted_alert.reach).to eq 16_092
              expect(promoted_alert.engagement["unique_clicks"]).to be_nil
              expect(promoted_alert.engagement["link_click"]).to eq "17"
            end
          end
        end
      end
