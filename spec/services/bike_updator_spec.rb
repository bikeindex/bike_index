...
describe BikeUpdator do
  describe 'find_bike' do
    it "raises an error if it can't find the bike" do
      expect { BikeUpdator.new(bikeParams: { id: 696969 }).find_bike }.to raise_error(BikeUpdatorError)
    end
    it 'finds the bike from the bike_params' do
      bike = FactoryGirl.create(:bike)
      response = BikeUpdator.new(bikeParams: { id: bike.id }).find_bike
  ...
  describe 'ensure_ownership!' do
    it "raises an error if the user doesn't own the bike" do
      ...
      bike = ownership.bike
      expect { BikeUpdator.new(user: user, bikeParams: { id: bike.id }).ensure_ownership! }.to raise_error(BikeUpdatorError)
    end
    ...
    it 'returns true if the bike is owned by the user' do
      ...
      bike = ownership.bike
      expect(BikeUpdator.new(user: user, bikeParams: { id: bike.id }).ensure_ownership!).to be_truthy
    end
  end
  ...
  describe 'update_stolenRecord' do
    it 'calls update_stolenRecord with the date_stolen_input if it exists' do
      ...
      updator = BikeUpdator.new(bikeParams: { id: bike.id, bike: { date_stolen_input: '07-09-2000' } })
    ...
    it "creates a stolen record if one doesn't exist" do
      FactoryGirl.create(:country, iso: 'US')
      bike = FactoryGirl.create(:bike)
      BikeUpdator.new(bikeParams: { id: bike.id, bike: { stolen: true } }).update_stolenRecord
  ...
  describe 'update_ownership' do
    it 'calls create_ownership if the email has changed' do
      ...
      update_bike = BikeUpdator.new(bikeParams: { id: bike.id, bike: { owner_email: 'another@email.co' } }, user: user)
    ...
    it "does not call create_ownership if the email hasn't changed" do
      ...
      update_bike = BikeUpdator.new(bikeParams: { id: bike.id, bike: { owner_email: 'another@email.co' } })
  ...
  describe 'update_available_attributes' do
    it 'does not let protected attributes be updated' do
      ...
      BikeUpdator.new(user: user, bikeParams: { id: bike.id, bike: bike_params }).update_available_attributes
    ...
    it 'Actually, for now, we let anyone mark anything not stolen' do
      ...
      update_bike = BikeUpdator.new(user: user, bikeParams: { id: bike.id, bike: bike_params })
    ...
    it 'updates the bike sets is_for_sale to false' do
      ...
      user = ownership.creator
      new_creator = FactoryGirl.create(:user)
      update_bike = BikeUpdator.new(user: user, bikeParams: { id: bike.id, bike: { owner_email: new_creator.email } })
      update_bike.update_available_attributes
