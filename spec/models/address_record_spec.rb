...
RSpec.describe AddressRecord, type: :model do
  ...
  describe "clean_city" do
    let(:address_record) { AddressRecord.new(city:, country_id: Country.united_states_id, region_record_id:, skip_geocoding: true) }
    let(:region_record_id) { nil }
    ...
    context "with city containing state abbreviation" do
      ...
      it "removes state abbreviation and assigns region" do
        ...
        expect(address_record.city).to eq "Sacramento"
        expect(address_record.region_record_id).to eq california.id
      end
    end
    ...
    context "with city containing non-matching state abbreviation" do
      ...
      it "does not remove the abbreviation" do
        address_record.valid?
        expect(address_record.city).to eq "Sacramento, NY"
      end
    end
    ...
    context "with non-US country" do
      let(:address_record) { AddressRecord.new(city: "Amsterdam, NH", country_id: Country.netherlands.id, skip_geocoding: true) }

      it "does not modify city" do
        address_record.valid?
        expect(address_record.city).to eq "Amsterdam, NH"
      end
    end
