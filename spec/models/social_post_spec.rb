...
RSpec.describe SocialPost, type: :model do
  ...
  describe "admin_search" do
    ...
    it "finds the post" do
      expect(social_post.kind).to eq "stolen_post"
      expect(SocialPost.admin_search("@PPBBIKETHEF").pluck(:id)).to eq([social_post.id])
      # This matches the post body - but if passed a number, admin_search only matches the actual bike id
      ...
      # Ensure passing a post id finds the post, and passing most of a post id finds the post too!
  ...
  describe "#repost?" do
    context "given no original post" do
      it "returns false" do
        social_post = FactoryBot.create(:social_post, original_post: nil)
    ...
    context "given an original post" do
      it "returns true" do
        social_post = FactoryBot.create(:social_post, original_post: nil)
        repost = FactoryBot.create(:social_post, original_post: social_post)
        expect(social_post).to_not be_repost
  ...
  context "platform response present" do
    ...
    it "gives us the responses we want" do
      expect(social_post.poster).to eq "stolenbikereg"
      expect(social_post.posted_at.to_i).to eq 1497366412
      expect(social_post.poster_avatar).to eq "https://pbs.twimg.com/profile_images/505773652646711296/bTYbvFTy_normal.jpeg"
      expect(social_post.poster_name).to eq "BikeIndex Portland"
      expect(social_post.posted_image).to be_blank
    end

    describe "platform response access" do
      ...
      it "returns what we expect" do
        expect(social_post.poster).to eq "bikeindexstolen"
        ...
        expect(social_post.poster_avatar).to eq "https://pbs.twimg.com/profile_images/505774915660705793/UpEjqnux_normal.jpeg"
      end
    end
  end

  describe "send_post" do
    let(:social_account) { FactoryBot.create(:social_account, account_info: {stuff: ""}) }
    let(:social_post) { SocialPost.create(body: "testing new system", social_account: social_account, kind: "app_post") }
    it "creates a post" do
      expect(social_account).to receive(:post) { {something: "ffff"} }
