...
RSpec.describe ImpoundRecord, type: :model do
  describe "validations" do
    ...
    it "marks the bike impounded only once" do
      ...
      bike.impound_records.create(user: user, bike: bike)
      bike.reload
      ...
      new_impound_record = bike.impound!(user: user, bike: bike)
      bike.reload
      expect(bike.impound_records.count).to eq 1
    ...
    context "user not authorized" do
      ...
      it "does not impound" do
        ...
        expect(bike.impounded?).to be_falsey
        impound_record = bike.impound_records.create(user: user, bike: bike)
        bike.reload
        expect(bike.impounded?).to be_falsey
