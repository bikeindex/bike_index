...
RSpec.describe OrganizationStatus, type: :model do
  ...
  describe "find_or_create_current" do
    ...
    it "returns the current status" do
      ...
      expect(OrganizationStatus.find_or_create_current(organization)).to have_attributes_with_time_within target_attrs
    ...
    context "when kind is updated kind" do
      it "creates a second status" do
        ...
        expect(organization_status1).to have_attributes_with_time_within target_attrs
        ...
        expect(organization_status2).to have_attributes_with_time_within target_attrs.merge(kind: "bike_shop", start_at: new_updated_at)

        expect(organization_status1.reload).to have_attributes_with_time_within target_attrs.merge(end_at: new_updated_at)
      end
    end
    ...
    context "with new POS kind, then deletion" do
      it "creates a status" do
        ...
        organization_status2 = OrganizationStatus.find_or_create_current(organization)
        expect(organization_status2.id).to_not eq organization_status1.id
        expect(organization_status2).to have_attributes_with_time_within target_attrs.merge(pos_kind: "lightspeed_pos", start_at: new_updated_at)
    ...
    context "when organization is deleted" do
      it "ends the existing status" do
        ...
        expect(organization_status1.reload).to have_attributes_with_time_within target_attrs.merge(end_at: new_updated_at, organization_deleted_at: new_updated_at)
        ...
        organization_status2 = OrganizationStatus.find_or_create_current(organization)
        expect(organization_status2.id).to_not eq organization_status1.id
        expect(organization_status2).to have_attributes_with_time_within target_attrs.merge(start_at: organization.updated_at)
