...
RSpec.describe SocialAccount, type: :model do
  ...
  describe "#fetch_account_info" do
    ...
    it "sets the account info from fetch_account_info on save" do
      ...
      expect(social_account).to receive(:platform_user).and_return({screen_name: "foo", created_at: "Sun Jun 22 20:46:35 +0000 2014"})
    ...
    it "does nothing if account info is present" do
      ...
      allow(social_account).to receive(:platform_user).and_return({screen_name: "foo"})

      social_account.fetch_account_info

      expect(social_account).to_not have_received(:platform_user)
  ...
  it "delegates class methods to the client instance", vcr: true do
    ...
    post_id = 1170061123191791622
    status = SocialAccount.get_post(post_id)
    expect(status).to be_an_instance_of(Twitter::Tweet)
    expect(status.id).to eq(post_id)
  end
end
