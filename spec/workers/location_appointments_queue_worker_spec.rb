...
RSpec.describe LocationAppointmentsQueueWorker, type: :job do
  ...
  describe "perform" do
    ...
    it "sets the correct number of appointments on deck" do
      #
      # commented out for the intial demo :/
      #

      # This is gross, but we need ticket numbers in here, so...
      # [1169, 1174, 1180, 1173, 1172].each do |ticket_number|
      #   Ticket.create(number: ticket_number,
      #                 location: location,
      #                  appointment: Appointment.find_by_ticket_number(ticket_number))
      # end
      ...
      # expect(Ticket.paging_or_on_deck.pluck(:appointment_id)).to eq([appt1.id, appt3.id, appt4.id])
