...
RSpec.describe FetchMailchimpMembersWorker, type: :job do
  ...
  describe "perform" do
    context "organization" do
      ...
      it "creates the given number of mailchimp_datums" do
        ...
        VCR.use_cassette("fetch_mailchimp_members_worker-success", match_requests_on: [:path]) do
          instance.perform("organization", 0, false, 1)
        end
    ...
    #   let(:target_data) { {stuff: "fff"} }
    #   let(:user) { FactoryBot.create(:user, email: "party@stuff.com") }
    #   let!(:mailchimp_datum) { MailchimpDatum.create(user: user, mailchimp_updated_at: Time.current - 2.hours, data: {lists: ["organization"]}) }
    ...
    #       instance.perform("individual", 0, true, 1)
    #     end
    #     # Because we're enqueue_all_pages
    #     expect(described_class.jobs.count).to be > 1000
    #     expect(MailchimpDatum.count).to eq 1
    #     mailchimp_datum = MailchimpDatum.last
    #     expect(mailchimp_datum.email).to eq "party@stuff.com"
    #     expect(mailchimp_datum.user_id).to be_blank
    #     expect(mailchimp_datum.data).to eq target_data
    #   end
    # end
  end
end
