...
If `bin/dev` isn't responding *and* this run will create a new PR (or recapture screenshots), start it in the background with `bin/dev` — Tailwind and JS assets need to rebuild before any commit. For pure body-update runs you can skip starting `bin/dev`.
...
If there are no frontend changes, skip steps 3–4 and 6–7 — step 5 still creates/updates the PR, just without a Screenshots section.
