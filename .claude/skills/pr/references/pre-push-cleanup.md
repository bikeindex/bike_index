...
Scope specs the same way — the ones covering what the branch changed, never a bare `bundle exec rspec` or a whole top-level directory (see the `rspec-testing` skill). CI runs the full suite; a green PR isn't your job to prove locally. Scoping down is the risk here too: walk the changed `app/` paths to their `spec/` siblings rather than picking the specs the change feels like it's about, since a caller you edited in passing has its own. Adding a key to `HeaderTagHelper#header_tags_component_options` went out red that way — `spec/helpers/header_tag_helper_spec.rb` asserts the whole hash with `eq`, and the component and request specs that looked like the subject all passed.
