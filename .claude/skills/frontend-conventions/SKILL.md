...
- **A `@layer components` rule loses to a utility class on the same element**, whatever the specificity — so a hand-written rule that sets `display` is inert against an element carrying `tw:grid` or `tw:flex`, silently. Rather than reaching for `!`, point the rule at a wrapper that carries no utility for that property: `.search-results-body` in `application.css` exists for exactly that, wrapping rows whose `<ul>` brings its own `tw:grid`. Reach for a `@custom-variant` instead when the state is worth naming for templates to use inline (`search-all-checked` is the pattern) — those emit as utilities and need no `!` either.
