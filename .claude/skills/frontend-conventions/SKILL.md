...
- **Run `bin/update_markup_digests` after editing markup that renders inside a fragment cache**, rather than computing a digest by hand. A `MARKUP_DIGEST` follows `render X::Component` *and* `render "partial"` transitively — so editing anything either tree renders, however far down, moves it. It hashes each of those files' *paths* alongside their contents, so moving or renaming one stales every digest above it even though the markup is byte-identical. `spec/services/markup_digest_spec.rb` is what catches a stale one.
- **A digest has two homes, and a view template is the one that's easy to miss.** A component (`SharedBlocks::Footer`, `SharedBlocks::Navbar::Wrapper`) assigns `MARKUP_DIGEST = "…"` and folds the constant into its cache key. A template carries the same assignment in a comment — `<%# MARKUP_DIGEST = "" %>`, or `-# MARKUP_DIGEST = ""` in haml — and needs nothing else: Rails already folds the template's own digest into the fragment key, so rewriting the comment moves it. That's the fix whenever a view caches around a component (`app/views/bikes/_bike.html.erb` is the pattern), since Rails' digest follows the view's partials but stops dead at `render Foo::Component.new`. It does nothing under `skip_digest: true`, where the key has to carry the digest itself.
