...
- **`UI::Table` caches per cell, and its `cache_key` has to cover the column *set*.** A cell's key is the column's position, so a column added or dropped without a matching term in `cache_key` serves every later column the neighbour's markup — `admin/payments/_table.html.erb` is the pattern, and `admin/logged_searches/_table.html.erb` keys on the two sorts that add a column. When the column set follows a record's settings rather than a flag, pass `cache_rows: true` and put that record in `cache_key` — `Pages::Org::SearchResults::BikesTable` is the pattern. The rest of the key has two inputs and one exclusion: the record (automatic), every *other* record the cell renders (`cache_records:`, which mirrors the controller's `includes` — a cell reading an association the controller doesn't preload is an N+1 and a stale cell at once; one that *can't* be preloaded, because a method re-queries over it like `TheftAlert#bike`, stays out of the key rather than costing a query per row on every hit), and **never** `sort_state.search_params`, which varies per request: wrap the table in `update-cached-sortable-links` and let it rewrite those hrefs client-side.
- **A cell shared across tables caches itself with `shared_cache_if`.** `Atoms::Admin::TableCells::User` is the pattern: its key holds only its own arguments, so the fragment one table writes is the fragment every other table reads, and `UI::Table` leaves any cell that renders one out of its own cache. The search link stays outside its `cache` block — the href carries the request's own params. Such a cell keeps its record out of the table's `cache_records` too, so the row's other cells don't bust with it.
