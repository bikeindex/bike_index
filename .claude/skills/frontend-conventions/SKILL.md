...
- **If a view file only renders a single component, consider rendering it from the controller instead** (`render Registrations::New::StartForm::Component.new(b_param:)`) and deleting the view file — the layout still wraps it. See `RegisterController`.
