...
- **A custom `@utility` has no fixed rank against a core one it collides with** — `tw:twfullbleed` emits after `tw:border`, `tw:twdev-only` before `tw:bg-white`, so a same-specificity tie can go either way. When yours has to win, take the specificity with `@variant` or `!` rather than assuming; `twdev-only` is the pattern. Grep `app/assets/builds/tailwind.css` for both selectors to see which lands first.
