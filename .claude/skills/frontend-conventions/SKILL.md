...
**What's banned is the component reaching out, not the number of arguments.** State the controller already owns can be named and passed as one value object — `ComponentStates::IndexState` (built in `Admin::BaseController#admin_index_state`) and `ComponentStates::SortState` (`ControllerHelpers#sort_state`) are the pattern — value objects live in `app/services/component_states/`. The component stays pure either way; a bundle just stops seventy views from re-listing the same seventeen assigns.
