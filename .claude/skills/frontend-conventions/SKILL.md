...
- **A form revealed by a collapse or action panel submits with `color: :primary`.** `:purple` is the trigger that opens the reveal, and the consequential action inside a modal or prompt. `Pages::Registrations::Show::ContactOwner` and its `CurrentAlerts::ClaimImpound` sibling are the pattern.
