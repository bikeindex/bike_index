...
- **Updating call sites after a signature change, match the whole render call, not the kwarg.** A file often renders two components that share an argument — `bike_access_panel` passes `current_organization:` to both its notification tables — so a blanket replace of the kwarg string drops it from the neighbour too. A required one then raises `ArgumentError` only where that branch renders, which the suite may never reach.
...
- **Dev-only markup inside a fragment cache renders unconditionally, classed `only-dev-visible`** — `SharedBlocks::HeaderTags` hides that class per request, so one cached fragment serves dev and non-dev viewers both. Never put `display_dev_info?` in a cache key, and never inject a `<style>` to hide it; `admin/notifications/_table.html.haml` is the pattern. The class only hides: a dev payload a non-dev shouldn't hold in their page source at all — `Pages::Org::BulkImportError`'s raw `import_errors` — keeps its `display_dev_info?` gate.
