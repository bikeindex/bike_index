...
- **`.only-dev-visible` is a tint, not a hide.** A dev-only record id is `lower_right: (->(record) { tag.span(record.id, class: "only-dev-visible") } if display_dev_info?)`, and a dev-only column sits inside `if display_dev_info?` — never an injected `<style>.only-dev-visible {display: none}</style>`. A partial's row cache is `cache_key: cache_fragment_name([...])` listing every local a cell reads, `display_dev_info?` included; `admin/notifications/_table.html.erb` is the pattern for both.
