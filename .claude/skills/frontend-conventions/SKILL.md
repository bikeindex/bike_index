...
- **Dev-only markup inside a fragment cache renders unconditionally with `only-dev-visible`** — `SharedBlocks::HeaderTags` hides that class for non-dev viewers, so one cached fragment serves both. Never put `display_dev_info?` in a cache key, and never inject a `<style>` to hide it. CSS hides but doesn't omit, so markup that shouldn't reach a non-dev's page source — anything on an org-facing page — keeps its `display_dev_info?` gate.
