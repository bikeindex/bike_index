...
- **A `@layer components` rule loses to a utility class on the same element**, whatever the specificity — so a hand-written rule that sets `display` is inert against an element carrying `tw:grid` or `tw:flex`, silently. A variant utility has no such problem, and is the first thing to reach for: `tw:group-[[busy]]:hidden` on the org search's rows beats the `tw:grid` beside it, off the results frame's `tw:group`. `@custom-variant` when the state is worth naming for templates to use inline (`search-all-checked` is the pattern); a `@layer components` rule only for what a variant can't reach, like a sibling `:has()` chain.
