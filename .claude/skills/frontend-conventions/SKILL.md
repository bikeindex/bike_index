...
- **Components that share markup both render a third that owns it, with slots for what differs** — never a subclass rendering its parent's template. `UI::Forms::Files::Picker`, rendered by `Files::Upload` and `Files::UploadMultiple`, is the pattern.
