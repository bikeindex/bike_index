...
- **Markup inside a fragment cache needs no digest of its own** — `ApplicationComponent` includes `ViewComponent::ExperimentallyCacheable`, which puts every component in Rails' template digest tree. A `<% cache %>` block in a view or a component then invalidates when anything it renders changes, however far down, and `render partial:` chains come free from Rails. Nothing to run before pushing, and nothing to commit.
- **A component that builds its own `cache_key` folds in `self.class.cache_digest`** (`SharedBlocks::Footer`, `Pages::Registrations::Show::Wrapper`) — a component's internal `cache` block gets no template digest from Action View, so the key has to carry one. Rails memoizes it per process.
- **A component you don't render as a literal constant needs a `# Template Dependency:` line.** The tracker reads one shape — a constant directly after `render` — so `render(inner_component)`, `render primary_menu`, a component pushed into a collection, or one referenced only for a constant all drop out of the digest, and their markup then goes stale inside every cache above them. Name the full class in a comment on the component that renders it (`# Template Dependency: SharedBlocks::Navbar::PrimaryMenu::Component`) and the tracker resolves it into the tree. `spec/components/application_component_spec.rb` fails on both a missing one and one left behind after its render moved away.
- **Two ways out of that coverage, both silent.** `cache(…, skip_digest: true)` drops the template digest entirely, so the key has to carry the rendered component's `cache_digest` itself — `welcome/index.html.erb` is the only caller, and the pattern. And a component that doesn't descend from `ApplicationComponent` is invisible to the tracker, so anything it renders can go stale inside a cache above it.
