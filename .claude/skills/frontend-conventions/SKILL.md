...
- **Dev-only markup inside a fragment cache renders unconditionally, classed `only-dev-visible`** — `SharedBlocks::HeaderTags` hides that class per request, so one cached fragment serves dev and non-dev viewers both. Never put `display_dev_info?` in a cache key, and never inject a `<style>` to hide it; `admin/notifications/_table.html.erb` is the pattern. The class only hides: a dev payload a non-dev shouldn't hold in their page source at all — `Pages::Org::BulkImportError`'s raw `import_errors` — keeps its `display_dev_info?` gate.
- **A `cache(...)` inside a `UI::Table` cell digests `UI::Table`'s template, not the partial's**, so a bare `cache(record.user)` shares its fragment with every other table caching that user. Take `cache_fragment_name` above the table and key the cell `cache([that, ...every input the cell reads], skip_digest: true)` — `admin/payments/_table.html.erb` is the pattern.
