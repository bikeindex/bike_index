...
- **Dev-only markup inside a fragment cache renders unconditionally with `only-dev-visible`** — `SharedBlocks::HeaderTags` hides that class for non-dev viewers, so one cached fragment serves both. Never put `display_dev_info?` in a cache key, and never inject a `<style>` to hide it. A partial's row cache is `cache_key: cache_fragment_name([...])` listing every local a cell reads; `admin/notifications/_table.html.erb` is the pattern.
