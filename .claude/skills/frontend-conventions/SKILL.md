...
**Styling legacy markup a component's *callers* own is a class in `app/assets/tailwind/bike_index_components.css`**, not an arbitrary-variant blob (`tw:[&_a.nav-link]:…`) in the component's Ruby — `.twdropdown`'s entry rules are the pattern. Take its resting and active fills from `UI::Button::Component::COLORS`/`ACTIVE_COLORS`, the way `UI::ButtonGroup::Component::CHIP_CLASSES` composes them, so a legacy item and the `UI::` component beside it agree.
