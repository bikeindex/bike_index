...
- **Two ways out of that coverage, both silent.** `cache(…, skip_digest: true)` drops the template digest entirely, so the key has to carry `cache_digest` itself (`welcome/index.html.erb` is the one caller). And a component that doesn't descend from `ApplicationComponent` is invisible to the tracker — anything it renders can go stale inside a cache above it.
