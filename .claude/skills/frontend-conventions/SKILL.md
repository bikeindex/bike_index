...
- **A `UI::Table` cell block is `instance_exec`'d against the component.** Inside `table.column ... do`, bare calls and `@ivar`s resolve on `UI::Table::Component`, not the view — reach state through the readers it exposes (`sort_state.search_params`, not `sortable_search_params`). Above the `UI::Table::Component.new` block the view's own helpers work; rewriting those too is churn.
