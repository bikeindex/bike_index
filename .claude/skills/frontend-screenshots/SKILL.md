...
- **A 200 doesn't prove the server is this checkout's.** Confirm `ruby bin/env --export` names a `WORKSPACE_ID` first — without one the curl reaches the main checkout. See the `sandbox-test-setup` skill. **Exception: the web sandbox** (`/home/user/bike_index`) has a single checkout and no workspace registry, so `WORKSPACE_ID` is empty and `$BASE_URL` is the default `http://localhost:3042` — correct there, not a reason to stop.
