...
**On a dev server that's been up a while, the group page stops picking up newly added scenarios** — it renders every *other* one, which reads as a broken preview rather than a stale registry (a restart clears it; a fresh server picks them up within a request or two). The bare `/rails/view_components/…` route stays current either way, since `config/initializers/lookbook.rb` patches `__vc_load_previews` to re-resolve through the autoloader — capture a new scenario there.
