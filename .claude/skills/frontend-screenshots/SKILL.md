...
- **A 200 doesn't prove the server is this checkout's.** Confirm `ruby bin/env --export` names a `WORKSPACE_ID` first — without one the curl reaches the main checkout, whose pages are another branch's and whose database is full of real user data. That worktree isn't set up; see the `sandbox-test-setup` skill.
