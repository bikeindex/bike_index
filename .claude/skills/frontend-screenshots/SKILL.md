...
- `curl -fs "$BASE_URL/" >/dev/null` — if not up, stop and ask the user to start `bin/dev` themselves. **Never start or stop `bin/dev`** — it's the user's process; your own copy can land on a different DB.
