...
**Re-measure once after that resize — a form-heavy page grows into it.** `/o/:slug/manage` reports 2051, then 2608 at that height, and never converges, because its rows reflow into the taller viewport. When the height moves, capture `fullPage` at the standard viewport instead of chasing it.
