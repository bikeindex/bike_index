class BikeIndex.LegacyStolenIndex extends BikeIndex
  constructor: ->
    @toggleMultiSearch() if window.location.href.match(/multi.?serial.?search/i)
    @initializeEventListeners()

  initializeEventListeners: ->
    $('#show_multi_search').click (e) =>
      e.preventDefault()
      @toggleMultiSearch()
    $('#multi_serial_search').change (e) =>
      @unlockSearch()
    $('#search_serials').click (e) =>
      e.preventDefault()
      @searchForSerials()
    $('#multiserial_fuzzy').click (e) =>
      e.preventDefault()
      @searchSerialsFuzzy()
