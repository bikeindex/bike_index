  ...
  def show
    ...
    if @ownership.claimable_by?(current_user)
      if @ownership.current
        ...
        flash[:success] = translation_with_args(:you_claimed_it, bike_type: bike.type)
        redirect_to edit_bike_url(bike)
      else
        flash[:error] = translation_with_args(:no_longer_your_bike, bike_type: bike.type)
        redirect_to bike_url(bike)
      end
    else
      flash[:error] = translation_with_args(:not_your_bike, bike_type: bike.type)
      redirect_to bike_url(bike)
    end
  end
