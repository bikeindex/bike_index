class SessionsController < ApplicationController
  ...
  def create_magic_link
    ...
    if user.present?
      ...
      flash[:success] = translation_with_args(:link_sent)
      redirect_to root_path
    else
      flash[:error] = translation_with_args(:user_not_found)
  ...
  def create
    ...
    if @user.present?
      if @user.authenticate(permitted_parameters[:password])
        ...
        flash.now[:error] = translation_with_args(:invalid_email_or_password)
