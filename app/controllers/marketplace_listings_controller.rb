...
class MarketplaceListingsController < ApplicationController
  ...
  def update
    if @marketplace_listing.publish!
      flash[:success] = translation(:item_published_for_sale, item_type: @marketplace_listing.item_type)
    else
  ...
  def ensure_user_allowed_to_edit!
    ...
    flash[:error] = translation(:you_dont_own_that, item_type: @marketplace_listing.item_type)
