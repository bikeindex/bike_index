  ...
  class BaseController < ApplicationController
    ...
    def ensure_member!
      if current_user && current_user.member_of?(current_organization)
        ...
        flash[:success] = translation(:accept_tos_for_orgs, scope: t_scope(__method__))
      ...
      flash[:error] = translation(:not_a_member_of_that_org, scope: t_scope(__method__))
      redirect_to user_root_url and return
    end

    def ensure_admin!
      return true if current_user && current_user.admin_of?(current_organization)
      flash[:error] = translation(:must_be_org_admin, scope: t_scope(__method__))
    ...
    def ensure_ambassador_authorized!
      if current_organization&.ambassador?
        ...
        flash[:error] = translation(:must_be_ambassador, scope: t_scope(__method__))
      else
        flash[:error] = translation(:must_be_in_ambassador_org, scope: t_scope(__method__))
      end
    ...
    def ensure_not_ambassador_organization!
      ...
      flash[:error] = translation(:must_be_an_admin, scope: t_scope(__method__))
    ...
    def t_scope(method_name)
      [:controllers, :organized, :base, method_name.to_sym]
    end
  end
end
