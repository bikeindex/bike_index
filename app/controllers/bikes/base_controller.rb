...
class Bikes::BaseController < ApplicationController
  ...
  def ensure_user_allowed_to_edit
    ...
    if @bike.current_impound_record.present?
      error = if @bike.current_impound_record.organized?
        translation_with_args(:bike_impounded_by_organization, bike_type: type, org_name: @bike.current_impound_record.organization.name,
          scope: [:controllers, :bikes, :ensure_user_allowed_to_edit])
      else
        translation_with_args(:bike_impounded, bike_type: type,
          scope: [:controllers, :bikes, :ensure_user_allowed_to_edit])
      end
    elsif current_user.present?
      error = translation_with_args(:you_dont_own_that, bike_type: type,
        scope: [:controllers, :bikes, :ensure_user_allowed_to_edit])
    else
      store_return_to
      error = if @current_ownership && @bike.current_ownership.claimed
        translation_with_args(:you_have_to_sign_in, bike_type: type,
          scope: [:controllers, :bikes, :ensure_user_allowed_to_edit])
      else
        translation_with_args(:bike_has_not_been_claimed_yet, bike_type: type,
          scope: [:controllers, :bikes, :ensure_user_allowed_to_edit])
      end
    end
  ...
  def assign_bike_stickers(bike_sticker)
    ...
    unless bike_sticker.present?
      return flash[:error] = translation_with_args(:unable_to_find_sticker, bike_sticker: bike_sticker,
    ...
    if bike_sticker.errors.any?
      ...
      flash[:success] = translation_with_args(:sticker_assigned, bike_sticker: bike_sticker.pretty_code, bike_type: @bike.type,
        scope: [:controllers, :bikes, :assign_bike_stickers])
    end
  end
