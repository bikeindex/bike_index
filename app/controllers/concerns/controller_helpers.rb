...
module ControllerHelpers
  ...
  def authenticate_user(translation_key: nil, translation_args: {}, flash_type: :error)
    ...
    if current_user&.confirmed?
      ...
      unless force_sign_up # Force signup doesn't show a flash message
        flash[flash_type] = translation_with_args(
          translation_key,
          **translation_args,
  ...
  def handle_target(target)
    ...
    when "password_reset"
      flash[:success] =
        translation_with_args(:reset_your_password,
          scope: [:controllers, :concerns, :controller_helpers, __method__])
  ...
  #
  #   flash[:error] = translation_with_args(:not_your_lock)
  #
  # maps to controllers.locks.find_lock.not_your_lock.
  def translation_with_args(key, scope: nil, controller_method: nil, **kwargs)
  ...
  def sign_in_if_not!
    ...
    store_return_to
    flash[:notice] = translation_with_args(:please_sign_in,
      scope: [:controllers, :concerns, :controller_helpers, __method__])
  ...
  def require_member!
    ...
    flash[:error] = translation_with_args(:not_an_org_member, scope: [:controllers, :concerns, :controller_helpers, __method__])
    redirect_to(my_account_url) && return
  end

  def require_admin!
    return true if current_user.admin_of?(current_organization)
    flash[:error] = translation_with_args(:not_an_org_admin, scope: [:controllers, :concerns, :controller_helpers, __method__])
    redirect_to(my_account_url) && return
  end
  ...
  def require_index_admin!
    ...
    flash[:error] = translation_with_args(:not_permitted_to_do_that, scope: [:controllers, :concerns, :controller_helpers, __method__])
  ...
  def ensure_member_of!(passed_organization)
    if current_user&.member_of?(passed_organization)
      ...
      flash[:success] = translation_with_args(:accept_tos_for_orgs,
        scope: [:controllers, :concerns, :controller_helpers, __method__])
    ...
    flash[:error] = translation_with_args(:not_a_member_of_that_org,
      scope: [:controllers, :concerns, :controller_helpers, __method__])
    redirect_to(user_root_url) && return
  end
