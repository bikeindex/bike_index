class PublicImagesController < ApplicationController
  ...
  def create
    ...
    flash[:error] = translation_with_args(:cannot_create)
  ...
  def update
    if params[:kind].present? # We're only updating kind
      ...
      redirect_to edit_bike_url(@public_image.imageable), notice: translation_with_args(:image_updated)
    else
      render :edit
    end
  end
  ...
  def destroy
    ...
    if imageable_type == "MailSnippet"
      flash[:error] = translation_with_args(:cannot_delete)
      redirect_to(admin_organization_custom_layouts_path(imageable_id)) && return
    end
    @public_image.destroy
    flash[:success] = translation_with_args(:image_deleted)
  ...
  def ensure_authorized_to_update!
    ...
    unless current_user_image_authorized?(@public_image)
      flash[:error] = translation_with_args(:no_permission_to_edit)
