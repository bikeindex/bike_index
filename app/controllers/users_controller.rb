class UsersController < ApplicationController
  ...
  def resend_confirmation_email
    ...
    if user_subject.present?
      ...
      flash[:success] = I18n.t(:resending_email)
    else
      flash[:error] = I18n.t(:please_sign_in)
    end
  ...
  def confirm
    ...
    if @user.confirmed?
      flash[:success] = I18n.t(:already_confirmed)
  ...
  def send_password_reset_email
    ...
    if @user.present?
      flash[:error] = I18n.t(:reset_just_sent_wait_a_sec) unless @user.send_password_reset_email
    else
  ...
  def update_password_with_reset_token
    if @user.present? && @user.update(permitted_password_reset_parameters)
      flash[:success] = I18n.t(:password_reset_successfully)
  ...
  def show
    ...
    unless user == current_user || @user.show_bikes
      redirect_to(my_account_url, notice: I18n.t(:user_not_sharing)) && return
    end
    @page = params[:page] || 1
    ...
    if @user.present? && params[:user].present? && @user.update(permitted_parameters)
      if params.dig(:user, :terms_of_service).present?
        if InputNormalizer.boolean(params.dig(:user, :terms_of_service))
          flash[:success] = I18n.t(:you_can_use_bike_index)
          redirect_to(my_account_url) && return
        else
          flash[:notice] = I18n.t(:accept_tos)
        ...
        if InputNormalizer.boolean(params.dig(:user, :vendor_terms_of_service))
          ...
          flash[:success] = if @user.memberships.any?
            I18n.t(:you_can_use_bike_index_as_org, org_name: @user.memberships.first.organization.name)
          else
            I18n.t(:thanks_for_accepting_tos)
          end
          redirect_to(user_root_url) && return
        else
          redirect_to(accept_vendor_terms_path, notice: I18n.t(:accept_tos_to_use_as_org)) && return
        end
      end
    end
  ...
  def unsubscribe
    ...
    if @user.blank?
      flash[:success] = I18n.t(:successfully_unsubscribed)
      redirect_to(user_root_url) && return
    end
  end
  ...
  def find_user_from_token_for_password_reset!
    ...
    remove_session
    flash[:error] = @user.blank? ? I18n.t(:does_not_match_token) : I18n.t(:token_expired)
