...
module Pages
  module Register
    module AcknowledgmentProgress
      # The eyebrow above each e-vehicle acknowledgment step, counting the review the
      # pages end at as the last one
        ...
        def call
          content_tag(:p, class: "tw:mb-4 tw:flex tw:items-center tw:gap-1.5 tw:text-xs tw:font-bold tw:text-gray-500 tw:dark:text-gray-400") do
            safe_join([
              ...
              translation(".progress_saved", number: @number, total: @total)
            ], " ")
          end
        end
      end
