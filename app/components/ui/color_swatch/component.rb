...
module UI
  module ColorSwatch
    ...
    class Component < ApplicationComponent
      BASE_CLASSES = "tw:inline-block tw:rounded-xs tw:border tw:border-black/20 tw:align-middle"

      SIZES = {sm: "tw:h-3 tw:w-3", md: "tw:h-6 tw:w-6"}.freeze

      def initialize(display: nil, name: nil, size: :md)
      ...
      def call
        content_tag(:span, "", class: "#{BASE_CLASSES} #{SIZES[@size]}", style: swatch_style, title: @name)
      end
