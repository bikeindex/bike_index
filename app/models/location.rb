class Location < ActiveRecord::Base
  def self.old_attr_accessor
    %w(name organization_id organization zipcode city state_id country_id street phone email latitude longitude shown).map(&:to_sym).freeze
  end  
  acts_as_paranoid
