...
class SocialAccount < ApplicationRecord
  ...
  def fetch_account_info
    ...
    self.account_info = twitter_user
    self.created_at = TimeParser.parse(account_info["created_at"])
    account_info
  end
  ...
  def repost(post_id)
    ...
    twitter_client.retweet(post_id).first
  rescue Twitter::Error::Unauthorized, Twitter::Error::Forbidden => err
    set_error(err.message)
    nil
  end
