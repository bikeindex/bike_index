...
class StolenRecord < ApplicationRecord
  ...
  class << self
    ...
    def corrected_date_stolen(date = nil)
      date = BinxUtils::TimeParser.parse(date) || Time.current
      year = date.year
  ...
  def add_recovery_information(info = {})
    ...
    self.recovered_at = BinxUtils::TimeParser.parse(info[:recovered_at], info[:timezone]) || Time.current

    update(
      current: false,
      recovered_description: info[:recovered_description],
      recovering_user_id: info[:recovering_user_id],
      index_helped_recovery: BinxUtils::InputNormalizer.boolean(info[:index_helped_recovery]),
      can_share_recovery: BinxUtils::InputNormalizer.boolean(info[:can_share_recovery])
    )
