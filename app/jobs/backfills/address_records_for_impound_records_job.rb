...
class Backfills::AddressRecordsForImpoundRecordsJob < ApplicationJob
  ...
  class << self
    ...
    def build_or_create_for(impound_record)
      ...
      existing_address_record = AddressRecord.where(kind: :impound_record).find_by(user_id: impound_record.user_id)
      ...
      impound_record.address_record = AddressRecord.new(kind: :impound_record)
