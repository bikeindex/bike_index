module CallbackJobs
  class AfterUserChangeJob < ApplicationJob
    ...
    def update_user_alerts(user)
      ...
      user.bike_organizations.invoiced.each do |organization|
    ...
    def process_bikes(user)
      if user.deleted?
        user.bike_ids.each { |id| BikeJobs::BikeDeleterJob.perform_async(id, false, user.id) }
      else
        user.bike_ids.each { |id| CallbackJobs::AfterBikeSaveJob.perform_async(id, true, true) }
      end
    end
  end
end
