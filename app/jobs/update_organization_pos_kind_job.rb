class UpdateOrganizationPosKindJob < ScheduledJob
  ...
  def self.calculated_pos_kind(organization)
    ...
    recent_bikes = bikes.where(created_at: (1.week.ago)..Time.current)
    ...
    if organization.bike_shop? && recent_bikes.count > 2
      return "does_not_need_pos" if organization.created_at < 1.week.ago ||
        bikes.where("bikes.created_at > ?", 1.year.ago).count > 100
    end
