module BikeJobs
  class UpdateTheftAlertFacebookJob < ScheduledJob
    ...
    def perform(promoted_alert_id = nil)
      ...
      if promoted_alert.facebook_data&.dig("ad_id").blank? || promoted_alert.failed_to_activate?
        return BikeJobs::ActivateTheftAlertJob.perform_async(promoted_alert_id)
      end

      Facebook::AdsIntegration.new.update_facebook_data(promoted_alert)

      return true unless promoted_alert.notify? &&
        promoted_alert.notifications.theft_alert_posted.none?

      # Perform inline rather than re-querying for objects
      Email::TheftAlertNotificationJob.new.perform(promoted_alert_id, "theft_alert_posted", promoted_alert)
    end
