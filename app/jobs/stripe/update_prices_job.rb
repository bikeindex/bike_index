...
class Stripe::UpdatePricesJob < ApplicationJob
  ...
  def product_membership_level
    ...
    membership_products = Stripe::Product.list({active: true, limit: 100})
      .select { |p| p.name.match?(/member/i) }

    @product_membership_level = [
      [membership_products.find { |p| p.name.downcase == "membership" }&.id, :basic],
      [membership_products.find { |p| p.name.match?(/plus|\+/i) }&.id, :plus],
      [membership_products.find { |p| p.name.match?(/patron/i) }&.id, :patron]
    ].to_h
  end
end
