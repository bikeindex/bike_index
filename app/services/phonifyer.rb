module Phonifyer
  ...
  def split_with_country_code(number)
    ...
    elsif country_code.length > 4 # If it's too long, just grab the first number
  ...
  def split_with_extension(number)
  ...
  private :strip_ignored_parts, :split_with_country_code, :split_with_extension
end
