...
// enclosing link at the selected photo's original, and swaps the active and
// inactive classes between the thumbnails.
...
export default class extends Controller {
  ...
  select (event) {
    ...
    // Both classes set border-color, so the inactive one has to come off
    this.thumbnailTargets.forEach((el) => {
      el.classList.toggle(this.activeClass, el === thumbnail)
      el.classList.toggle(this.inactiveClass, el !== thumbnail)
    })
  }
