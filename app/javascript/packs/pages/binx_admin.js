...
window.BinxAdmin = class BinxAdmin {
  init() {
    ...
    if ($("#invoiceForm").length > 0) {
      window.binxAdmin.initializeInvoiceForm();
    }
  ...
  initializeInvoiceForm() {
    ...
    $(".inputTriggerRecalculation").on("change paste keyup", e => this.updateInvoiceCalculations());
    return $("#invoiceForm .paidFeatureCheck input").on("change", e => this.updateInvoiceCalculations());
  };
  updateInvoiceCalculations() {
    let oneTimeCost, recurringCost;
    const recurring = $(".paidFeatureCheck input.recurring:checked").get().map(i => parseInt($(i).attr("data-amount"), 10));
    if (recurring.length > 0) {
      recurringCost = recurring.reduce((x,y) => x+y);
    } else {
      recurringCost = 0;
    }
    const oneTime = $(".paidFeatureCheck input.oneTime:checked").get().map(i => parseInt($(i).attr("data-amount"), 10));
    if (oneTime.length > 0) {
    ...
    const checked_ids = $(".paidFeatureCheck input:checked").get().map(i => $(i).attr("data-id"));
    return $("#invoice_paid_feature_ids").val(checked_ids);
  };
};
