window.BinxEditPhotos = class binxEditPhotos {
  init() {
  ...
  initializeImageUploads() {
    ...
    const finished_upload_template = $('#image-upload-finished-template').html();
    ...
    return $('#new_public_image').fileupload({
      ...
      add(e, data) {
        ...
        if (types.test(file.type) || types.test(file.name)) {
          ...
          return window.BikeIndexAlerts.add('error', `${file.name} is not a gif, jpeg, or png image file`);
        }
      },
      progress(e, data) {
        if (data.context) {
  ...
  pushImageOrder($sortable_container) {
    ...
    const array_of_photo_ids = (Array.from(sortable_list_items).map((list_item) => $(list_item).prop('id')));
    const new_item_order =
  ...
  updateImagePrivateness(e) {
    ...
    const url_target = `${$('#public_images').data('imagesurl')}/${id}/is_private`;
