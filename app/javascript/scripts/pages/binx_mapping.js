...
export default class BinxMapping {
  ...
  fitMap () {
    ...
    // Fit to markers
    for (const marker of Array.from(this.markersRendered)) {
      if (marker) {
  ...
  boundingBoxParams () {
    ...
    const bounds = binxMap.getBounds()
    return [
  ...
  renderAddressSearch () {
    ...
    this.searchBox.addListener('places_changed', function () {
      const places = binxMapping.searchBox.getPlaces()
      if (places.length === 0) {
  ...
  clearMarkers () {
    ...
    if (binxMapping.markersRendered.length) {
      for (const i of Array.from(binxMapping.markersRendered)) {
  ...
  addMarkers ({ fitMap = false, renderAddressSearch = true }) {
    ...
    while (binxMapping.markerPointsToRender.length > 0) {
      const point = binxMapping.markerPointsToRender.shift()
      const markerId = point.id
      const markerOpacity = binxMapping.kind == 'parking_notifications' ? 0.6 : 1
