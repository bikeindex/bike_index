...
export default class BinxAppOrgParkingNotificationMapping {
    ...
    $('#recordsTable, #parking-notification-nav-links').on(
      ...
      (e) => {
        ...
        if ($target.attr('data-urlparams')) {
          e.preventDefault()
          const urlOpts = $target.attr('data-urlparams').split(',')
  ...
  urlParamsForOpts (opts = []) {
    ...
    const urlParams = window.periodSelector.urlParamsWithNewPeriod()
  ...
  addTableMapLinkHandler () {
    $('#recordsTable').on('click', '.map-cell a', (e) => {
      ...
      const marker = _.find(binxMapping.markersRendered, function (marker) {
  ...
  mapPopup (point) {
    const record = _.find(binxAppOrgParkingNotificationMapping.records, [
      'id',
      point.id
    ])
    let tableTop =
