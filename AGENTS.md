...
- **A view component subclasses `ApplicationComponent` and nothing else** — never another component (a *preview* subclassing another preview is fine). A subclass's cache digest folds in the parent's Ruby, template and sidecars, so the parent's edits bust the child's fragments and a change meant for one renders in both. Duplicate the methods, or give both an object to call; `spec/components/application_component_spec.rb` fails on a second superclass. `BikeCard` and `BikeListItem` are the duplicated pair — and the copy each renders lives in its own sidecar, since a borrowed `component_translation_scope` is outside the borrower's digest.
