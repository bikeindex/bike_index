...
- **An enum belongs to the model that can reach every one of its values.** Aliasing another model's enum constant (`HotSheet::DELIVERY_STATUS_ENUM = Notification::DELIVERY_STATUS_ENUM`) gives each model the other's states as permanently-false predicates and empty scopes, and turns a value either side adds into a merge conflict over the same integer. Declare the enum literally on each model; share the pieces that really are common, like `Notification::UNDELIVERABLE_ERRORS`.
