...
- **Preview a user-authored HTML body through the model's `#body_stripped`** (`BugReport`, `MailSnippet`), not `strip_tags` in the view. `Binxtils::InputNormalizer.plain_text` decodes entities and collapses whitespace, so `truncate` needs no `escape: false` and a multi-line body reads as one line.
