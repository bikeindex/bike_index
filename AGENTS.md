...
- **A spam verdict goes through `SpamEstimator::Text.certain_spam?`, never `estimate`.** `SpamEstimator::Bike` weights the text score per field (0.2 for `frame_model`), so a verdict routed through `estimate` arrives as 20 and nothing is flagged. `PHARMACY_REGEX` is scored against frame models and theft reports, so it holds only terms no real one uses ("pharmacy" and "pills" do, and live in `SEO_SPAM_REGEX`, which scores profiles alone).
