...
- **A `def call` component returns an html-safe string.** ViewComponent escapes an unsafe one and writes a `Kernel.warn` per render, so a `call` returning a bare `translation(...)` floods prod stderr and every spec run that reaches the page — wrap it in `h`, as `UI::TimeRange::Component` and `Pages::Registrations::Show::Subtitle::Component` do. A `tag`/`content_tag`/`safe_join`/`render` return is already safe.
