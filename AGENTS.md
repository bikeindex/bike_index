...
- **A component built but never rendered can't call a route helper.** Several components double as data objects — `Pages::Org::Search::Settings` is built three times purely to read column labels and filter groups off it — and any `*_path` on one raises `ViewComponent::ControllerCalledBeforeRenderError`, which names `#controller` rather than the helper. Build the URL in whichever component is actually rendering, off the values the data object exposes.
