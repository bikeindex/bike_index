...
- **An action that renders a component inline needs `force_html_response` when anything can redirect to it.** A component takes its content type from the request, and Turbo leads its `Accept` with `text/vnd.turbo-stream.html` — so a Turbo form submission redirected there answers as a turbo_stream, which Turbo discards without touching the DOM or logging anything, leaving the browser sitting on the submitting page. A template-rendering action is safe: lookup falls back to `.html.erb` and sets the type with it. Keep it a per-controller `before_action` — the app has real `*.turbo_stream.erb` templates that a blanket `request.format = :html` would break.
