...
- **`Ownership#send_email` doesn't answer whether the email goes out.** It only negates the `skip_email` column, which is written at create time and isn't reconciled against `calculated_send_email` until `Email::OwnershipInvitationJob` runs — so a bike marked `likely_spam` has `send_email` truthy right up until the job drops the mail. Assert on `ActionMailer::Base.deliveries` around a drain, not on the flag.
