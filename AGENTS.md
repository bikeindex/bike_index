...
**Don't hand-edit `config/locales/translation.*.yml` for a new key either** — the sync writes those four too. The one thing to know is that a lookup in another locale raises rather than falling back to English in test, so the three specs that render one (`shared_blocks/footer`, `shared_blocks/navbar/wrapper`, `shared_blocks/header_tags`, all in `:nl`) go red on a key the sync hasn't reached yet.
...
- **Fragment caches carry the locale; `Rails.cache.fetch` doesn't.** `ApplicationComponentHelper#cache` overrides the view helper to fold `I18n.locale` into every key, so components and views get it free and their own `cache_key` methods look locale-less. A `Rails.cache.fetch` in a service or model skips all of that — cache anything translated there and one locale serves another's copy, which `UserServices::MenuItemsOrg` did. The locale is a request param or `Accept-Language` as much as `user.preferred_language`, so a `current_user` in the key doesn't stand in for it.
