...
**Anything broader than a handful of files goes through `bin/ci`** — it runs what CI runs (lint, scan, then the suite via the parallel `bin/turbo_tests`) in one command, with `--tests` and `--lint` to run half. A serial `bundle exec rspec` over a directory takes many times longer, and `bin/turbo_tests` is also what compiles assets and keeps the test Redis off the dev database.
