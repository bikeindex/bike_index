...
- **A namespace under `Admin::` or `Pages::` shadows a top-level one of the same name** — admin controllers and components nest in `module Admin`, and every page component in `module Pages`, so `Admin::Users` hides the `Users::` jobs and `Pages::Search` hides the `Search::` controllers from every file inside them. Rename the collision away rather than prefixing call sites with `::`: job namespaces carry the `*_jobs` suffix (`app/jobs/user_jobs`, `bike_jobs`, `callback_jobs`, `image_jobs`, `strava_jobs`), `Pages::SamlTest` is flat so it can't hide the `Saml::` services, and the landing-page slugs live under `LandingPageOrganizations` so `Pages::LandingPages` can't hide them. **Adding a namespace? Check it first** — `Rails.application.eager_load!` then `Object.const_defined?("LandingPages", false)`. Nothing fails at boot: the shadowed constant only raises where something reads it, so one unlucky spec on one CI shard is the whole signal. Only `.rb` files are exposed — ViewComponent compiles a template with a *string* `class_eval`, so `Module.nesting` inside it is the component class alone and a bare `Saml::` there still reaches the top level.
