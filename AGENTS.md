...
- **A view component subclasses `ApplicationComponent` and nothing else** — never another component. A subclass inherits its parent's translation scope and sits inside its digest tree, so the two go stale together and an edit meant for one reaches both. Duplicate the methods, or give both an object to call; `spec/components/application_component_spec.rb` fails on a second superclass. `BikeCard` and `BikeListItem` are the duplicated pair.
