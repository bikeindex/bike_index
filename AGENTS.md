...
**Branch around `translation`, never inside its key.** `translation(found? ? ".found_at" : ".impounded_at")` hides both keys, so i18n-tasks reports them unused and they get deleted — write `found? ? translation(".found_at") : translation(".impounded_at")`, repeating the interpolation arguments rather than hoisting the key into a local. Save `i18n-tasks-use` for a key no branch can make literal, like `translation(".#{kind}")` over an open set. It's what hid `thread_show`'s `:removed_message`, which matched no key at all.
