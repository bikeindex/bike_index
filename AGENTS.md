...
- **A condition added to `User.no_email_bans` is paid on a whole-table scan.** `Email::NewsletterJob` runs `valid_only` over every user and `admin/dashboard/index.html.erb` counts it three times per load — so measure it against the production-derived `bikeindex_development`, not a seeded workspace DB. Correlating a subquery against the outer `users` scan cost 411ms where resolving the same ids on their own cost 179ms.
