...
- **The org sidebar isn't the `organized/` namespace.** `passive_organization` falls back to the user's default organization role on every request, so a member gets the sidebar — `body.with-org-sidebar`, standing in for the top navbar rather than sitting beside it — on `/my_account` or a bike page too, where `main_content_organized?` is false and the layout renders the general alert itself. Reasoning about "the org pages" from the controller namespace misses them.
