...
- **`app/components` has five top-level folders, and a new component goes in exactly one of them**: `ui` (the design system — reusable controls and primitives, of which `ui/forms` is every form control), `atoms` (small renderers reused across pages — `Atoms::Serial`, `Atoms::Admin::Badges::User`), `pages` (one namespace per route, `Pages::Admin::Bikes::Table`), `shared_blocks` (the chrome around a page — navbar, footer, alerts), and `emails`. Nothing else belongs at the top level. **A `ui` component may read domain constants** — `UI::Forms::Email` reads `EmailDomain::RESERVED_REGEX`, `UI::Forms::Turnstile` reads `EmailDomain.risky_email?` — so "it knows about a model" doesn't disqualify a control from `ui/forms`; rendering inside a form is what qualifies it.
