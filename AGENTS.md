...
- **A public form that mails an address or creates a record renders `SharedBlocks::Honeypot::Component`.** The field name `additional` is a wire contract, not a style choice — `User#looks_like_spam?`, `Feedback#looks_like_spam?` and `BikeServices::Register` all key on that exact string, and two controllers permit it by name — so don't parameterize it. Pass `form_builder:` when the form is scoped to a model (`user[additional]`), omit it for a top-level `params[:additional]`. A missing trap is invisible until the spam complaints arrive; `app/views/registrations/embed.html.haml` is the one public form still without one.
