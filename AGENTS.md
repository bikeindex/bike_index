...
- **A new namespace under `Admin::` shadows a top-level one of the same name**, for every admin controller and component — they all nest in `module Admin`. Before adding one, grep the admin tree for bare `Foo::` references; the survivors need a leading `::`, as `Admin::RegistrationSequence`'s components do for `::RegistrationSequence`. Plural component namespaces (`Admin::Users`, `Admin::Bikes`) sidestep it, since the models are singular.
