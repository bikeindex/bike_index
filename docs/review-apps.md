...
- **pghero query stats need `shared_preload_libraries`.** The `pg_stat_statements` extension is created, but `shared-db` doesn't set `shared_preload_libraries=pg_stat_statements`, so the `pghero:capture_query_stats` cron records nothing. To enable it, add `cmd: "postgres -c shared_preload_libraries=pg_stat_statements"` to the `db` accessory in `config/deploy.review.yml` and `kamal accessory reboot db`.
